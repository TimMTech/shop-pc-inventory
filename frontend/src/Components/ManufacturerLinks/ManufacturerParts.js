...
const ManufacturerParts = ({ parts, addSelection }) => {
  const { title } = useParams();

  const findParts = parts.filter((part) => part.manufacturer === title);

  return (
    <ManufacturerWrapper>
      <ManufacturerTitle>{title}</ManufacturerTitle>
...
const ManufacturerTitle = styled.h1`
  margin: 2rem 0;
  font-size: 3rem;
`;
